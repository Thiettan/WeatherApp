...
document.querySelectorAll(".dislike_button_container").forEach(domContainer => {
  // Read the comment ID from a data-* attribute.
  const commentID = parseInt(domContainer.dataset.commentid, 10);
  ReactDOM.render(f(DislikeButton, { commentID: commentID }), domContainer);
});*/

/*document.querySelectorAll('.clock_container')
  .forEach(domContainer => {
    // Read the comment ID from a data-* attribute.
    const commentID = parseInt(domContainer.dataset.commentid, 10);
    ReactDOM.render(
      g(Clock, { commentID: commentID }),
      domContainer
    );
  });*/
/*document.querySelectorAll(".clock_container").forEach(domContainer => {
